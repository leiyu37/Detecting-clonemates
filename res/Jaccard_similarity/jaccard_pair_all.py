...
def jaccard_pair_all(vcf_path, sampleX1, sampleX2, out_path):
    sample_i_dict = {}
    f_in = open(vcf_path)
    ...
    for line in f_in.readlines():
        if not line.startswith("#"):
            ...
            if (genoX1 in ["00", "01", "10", "11"]) and (genoX2 in ["00", "01", "10", "11"]):
                if "0" in genoX1:
                    set_x1.add("{}_{}_0".format(columns[0], columns[1]))
                if "1" in genoX1:
                    set_x1.add("{}_{}_1".format(columns[0], columns[1]))
                if "0" in genoX2:
                    set_x2.add("{}_{}_0".format(columns[0], columns[1]))
                if "1" in genoX2:
                    set_x2.add("{}_{}_1".format(columns[0], columns[1]))
...
#run the calculation
jaccard_pair_all(vcf_path, sampleX1, sampleX2, out_path)
